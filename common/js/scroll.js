$(document).ready(function () {
    $(".his_nav_tab li:eq(0)").find("a").addClass("spy");
    //첫번째 서브메뉴 활성화

    // $('.history').find('.his2020s','.his2010s',
    // '.his2000s','.his1990s','.his1980s','.his1970s').removeClass('boxMove');

    $(".his_nav_tab").find(".his_fir").addClass("boxMove");

    var smh = $(".his_nav_tab").offset().top; //메인 비주얼의 높이
    var h1 = $(".his_fir").offset().top - 300;
    var h2 = $(".his_sec").offset().top - 300;
    var h3 = $(".his_thi").offset().top - 300;
    var h4 = $(".his_fou").offset().top - 300;
    ...
    $(window).on("scroll", function () {
        ...
        if (scroll >= 0 && scroll < h1) {
            ...
            $(".his_nav_tab li:eq(0)").find("a").addClass("spy");
            //첫번째 서브메뉴 활성화
            $(".his_fir").addClass("boxMove");
            //첫번째 내용 콘텐츠 애니메이션
        } else if (scroll >= h1 && scroll < h2) {
            $(".his_nav_tab li:eq(1)").find("a").addClass("spy");
            //두번째 서브메뉴 활성화
            $(".his_sec").addClass("boxMove");
        } else if (scroll >= h2 && scroll < h3) {
            $(".his_nav_tab li:eq(2)").find("a").addClass("spy");
            //세번째 서브메뉴 활성화
            $(".his_thi").addClass("boxMove");
        } else if (scroll >= h3 && scroll < h4) {
            $(".his_nav_tab li:eq(3)").find("a").addClass("spy");
            //세번째 서브메뉴 활성화
            $(".his_fou").addClass("boxMove");
        }
    });

    $(".his_nav_tab ul li a").click(function (e) {
        //슬라이드메뉴에 있는 a를 클릭하면
        ...
        if ($(this).hasClass("link1")) {
            ...
            value = $(".his_fir").offset().top - 50; // 해당 콘텐츠의 상단의 거리~~ //off변경 set 이 컨텐츠 위에 거리를 알아서 계산해주는게 .offset().top을 기억해두렴!
        } else if ($(this).hasClass("tab2")) {
            value = $(".his_sec").offset().top - 50;
        } else if ($(this).hasClass("tab3")) {
            value = $(".his_thi").offset().top - 50;
        } else if ($(this).hasClass("tab4")) {
            value = $(".his_fou").offset().top - 50;
        }
